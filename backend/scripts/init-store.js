...
class KeyStoreInitializer {
  ...
  _initAdminUser() {
    ...
    return this._fabricClient
      .getUserContext(enrollmentOptions.enrollmentID, true)
      .then((userFromStore) => {
  ...
  _initCommonUser() {
    ...
    return this._fabricClient
      .getUserContext(registerOptions.enrollmentID, true)
      .then((userFromStore) => {
        if (userFromStore && userFromStore.isEnrolled()) {
        ...
        return this._fabricCAClient
          .register(registerOptions, this._adminUser)
          .then((secret) => {
            const enrollmentOptions = {
              enrollmentID: USER_ENROLMENT_ID,
            ...
            return this._enrollUser(enrollmentOptions)
              .then((user) => {
  ...
  _enrollUser(enrollmentOptions) {
    return this._fabricCAClient
      .enroll(enrollmentOptions)
      .then(enrollment => this._fabricClient.createUser({
